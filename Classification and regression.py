...
def continuous_dice(arr_corr, prob_inputs):
    # Inputs arr_corr (512x512), prob_inputs (3x512x512)
    arr_corr = np.asarray(arr_corr, dtype = np.float16)
    arr_corr_sep = []
    # Create 3x512x512 array of the corrected scans
    # The three arrays are binary maps for classes 0, 1, and 2 respectively
    for i in range(0,3):
        temp_2 = []
        for j in range(0,len(arr_corr)):
            temp = []
            for k in range(0, len(arr_corr[j])):
                if (arr_corr[j][k] == i):
                    temp.append(1.0)
                else:
                    temp.append(0.0)
            temp_2.append(temp)
        arr_corr_sep.append(temp_2)
    
    cont_dice = 0
    for i in range(1, len(prob_inputs)): # 2 iterations
        corr_array = resize_2d_array(arr_corr_sep[i])
        prob_array = resize_2d_array(prob_inputs[i])
        maximum = np.amax(prob_array)
        for j in range(0,len(prob_array)):
            prob_array[j] /= maximum 
        union = 0
        a = 0
        b = 0
        c_den = 0
        for j in range(0,len(prob_array)): # 512*512 iterations
            a += corr_array[j]
            b += prob_array[j]
            c_den += corr_array[j] * np.sign(prob_array[j])
            union += corr_array[j] * prob_array[j]
        if(c_den > 0):
            c = union/c_den
        else:
            c = 1
        cont_dice += 2 * union/(c*a + b)
    cont_dice /= 2 
    return cont_dice

def get_Dice_Score(val,training_data,im_data,cor_im_data,dice_score_save_loc,data_possibilities,ans_augmented,prob_inputs, ans_cont_dice):
    Dice_Scores = []
    dice_score_values = []
    dice_plot = []
    augmented = ''
    ...
    for i in tqdm(range(0,len(training_data))): #16 iterations
        ...
        for j in range(0,len(training_data[i])): #6 iterations
            ...
            for k in range(0,len(training_data[i][j])): #113 iterations
                ...
                #print("Prob input dimensions:")
                #print(len(prob_inputs),len(prob_inputs[0]), len(prob_inputs[0][0]),len(prob_inputs[0][0][0]))
                #print("Corr dimensions")
                #print(len(cor_im_data),len(cor_im_data[0]), len(cor_im_data[0][0]))
                if ans_cont_dice == 'y':
                    d_s = continuous_dice(cor_im_data[i][j][k], prob_inputs[i][j][k])
                else:
    ...
    if not os.path.exists(cwd + dice_score_save_folder_location + augmented + cont_dice):
        os.makedirs(cwd + dice_score_save_folder_location + augmented + cont_dice)
    np.save(cwd + dice_score_save_loc + augmented + cont_dice + "\\Dice_Scores",Dice_Scores)
    np.save(cwd + dice_score_save_loc + augmented + cont_dice + "\\Dice_Score_Values",dice_score_values)
    np.save(cwd + dice_score_save_loc + augmented + cont_dice + "\\Dice_Plot",dice_plot)
...
def get_image_data(Type, data_folder, ans_augmented, ans_feat_vec):
    ...
    # Data should now be a 16x6xzx512x512 (or 16x1xzx512x512) array
    return data    
...
def load_saved_dice_scores(location,ans_augmented, ans_cont_dice):
    augmented = ''
    if ans_augmented == 'y':
        augmented = "_augmented"
    cont_dice = ''
    ...
    for file in list_of_files:
        temp.append(np.load(cwd + location + augmented + cont_dice + "\\"+ file,allow_pickle=True))
...
def main():
    ...
    prob_inputs = get_image_data(data_type, 'prob', ans_augmented, ans_feat_vec)
    
    ans_cont_dice = input("Use continuous Dice score (y/n) -> ")
    
    ans = input("Re-calculate dice scores (y/n) -> ")
    if ans =="y":
        dice_scores, dice_score_values, dice_plot = get_Dice_Score(val, patient_data, image_data, Corrected_image_data,dice_score_save_folder_location,data_possibilities, ans_augmented, prob_inputs, ans_cont_dice)
    else:
        dice_scores, dice_score_values, dice_plot = load_saved_dice_scores(dice_score_save_folder_location,ans_augmented, ans_cont_dice)
